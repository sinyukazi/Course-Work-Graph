...
static char *generate_password(int length, const char *charset) {
...
static int password_strength(const char *p) {
    ...
    for (int i = 0; i < len; i++) {
        if (strchr(charset_lower,  p[i])) lower = 1;
        if (strchr(charset_upper,  p[i])) upper = 1;
        if (strchr(charset_digits, p[i])) digit = 1;
        if (strchr(charset_symbols, p[i])) sym = 1;
    }
    int variety = lower + upper + digit + sym;
    return len * variety;
}

static const char* strength_to_string(int score) {
...
static void on_generate_clicked(GtkButton *btn, gpointer data) {
    ...
    char cs[256] = {0};
    if (use_lower)   strcat(cs, charset_lower);
    if (use_upper)   strcat(cs, charset_upper);
    if (use_digits)  strcat(cs, charset_digits);
    if (use_symbols) strcat(cs, charset_symbols);

    gtk_text_buffer_set_text(w->text_buffer, "", -1);
    char out[4096] = {0};
    for (int i = 0; i < count; i++) {
        char *pwd = generate_password(length, cs);
        if (!pwd) continue;
        char line[256];
        snprintf(line, sizeof(line), "%d: %s\n", i + 1, pwd);
        strcat(out, line);
        if (i == count - 1) {
            int score = password_strength(pwd);
            gtk_label_set_text(w->label_strength, strength_to_string(score));
        }
        free(pwd);
    }
...
int main(int argc, char *argv[]) {
    ...
    g_signal_connect(btn, "clicked", G_CALLBACK(on_generate_clicked), w);
